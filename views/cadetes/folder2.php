...
if(r[0]!=null){
	//Verfificamos si el tipo de falta es grave
	if(r[3]=='A'||r[3]=='A*'){
		$('#nuevaFalta td:odd').show();
		$('#dialog-confirmacion').dialog('open');
		if(confirm("El Cadete Ingresara a Consejo Disciplinario")){
		alert("La Falta amerita Consejo Disciplinario");
		}
	}
...
<td class="cpd"><?php echo $pd; ?></td>
<td class="ctss" ><?php echo $tss; ?></td>
<td class="ctte"><?php echo $tte; ?></td>
<td class="ctss"><?php echo $tssc; ?></td>
<td class="ctte"><?php echo $ttec; ?></td>
</tr>
<tr>
...
<div class="toggler">
	...
	<form action="main.php?controller=faltas&action=registrarFalta" method="post">
        ...
        <hr/>
        <p>
        <label>Concejo Disciplinario:</label>
        </p>
        <p>
        <label>N:</label><input type="text" name="numero" />
        </p>
        <label>Ao:</label><input type="text" name="anio" />
        <hr/>
...
<!-- caso de faltas tipo A -->
<div id="dialog-faltasA" title="Registrar">
	<form action="main.php?controller=faltas&action=registrarTSS">
	<input name="idfc" id="idfc3" type="hidden"/>
	<P id="faltaTTE"></P>
	<p>
...
<div id="dialog-confirmacion" title="Empty the recycle bin?">
